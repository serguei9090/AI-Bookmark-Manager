...
import { Search, Settings, Globe, ExternalLink, Moon, Sun, ArrowLeft, LayoutGrid, Check, Folder } from 'lucide-react';
import { useAppContext } from '../store';
import { Bookmark } from '../types';

export function SimplePopupView() {
  const { bookmarks, folders, settings, setSettings, updateBookmark } = useAppContext();
  ...
  const filteredBookmarks = useMemo(() => {
    if (!searchQuery.trim()) {
      ...
      return [...bookmarks].sort((a, b) => b.dateAdded - a.dateAdded).slice(0, 6);
    }
    ...
    <div className="h-full flex flex-col bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans transition-colors duration-300">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-3 mb-4">
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            <Settings size={15} className={showSettings ? 'animate-spin-hover' : ''} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {showSettings ? (
          ...
          <form onSubmit={handleSaveSettings} className="space-y-4 pr-1 animate-fade-in">
            ...
            <div className="space-y-3">
              <div>
                ...
                <span className="block text-[9px] text-gray-500 dark:text-gray-450 mb-1 truncate" title={activeUrl}>
                  Using: {activeUrl}
                </span>
                <input 
                  type="text"
              ...
              <div>
                ...
                <input 
                  type="password"
                  placeholder={
                    settings.provider === 'gemini' ? 'Optional (uses local proxy)' :
                    settings.provider === 'ollama' || settings.provider === 'lmstudio' ? 'Optional' :
                    'Enter API Key...'
                  }
                  value={localApiKey}
              ...
              <div>
                ...
                <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-800 rounded-xl px-3 py-2 text-xs text-gray-500 font-mono">
                  {settings.provider === 'gemini' ? settings.model : `${settings.provider} / ${settings.model}`}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              {filteredBookmarks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center text-gray-400 dark:text-gray-650">
                  <Globe className="text-gray-300 dark:text-gray-700 mb-2" size={30} />
                  <span className="text-xs font-semibold">No bookmarks found</span>
                  <span className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">Try searching for a different keyword</span>
                </div>
              ) : (
