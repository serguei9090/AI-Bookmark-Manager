import { AlertTriangle, Check, CopyX, ShieldCheck, Trash } from "lucide-react";
...
export function DuplicatesView() {
	...
	const handleBulkDeDuplicate = () => {
		...
		if (
			...
			alert(
				`Done! Removed ${toRemoveIds.length} redundant ${toRemoveIds.length === 1 ? "copy" : "copies"}. Originals preserved.`,
			);
		}
	};
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-gray-500 text-sm mt-1">
							Scans exact URLs to identify repetitive listings safely.
						</p>
						{totalGroupCount > 0 && (
							<div className="flex items-center gap-2 mt-2 flex-wrap">
								<span className="inline-flex items-center gap-1.5 bg-red-100 dark:bg-red-950/50 text-red-700 dark:text-red-300 text-xs font-bold px-3 py-1 rounded-full border border-red-200 dark:border-red-900/40">
									<CopyX size={12} />
									{totalDuplicateCount} duplicate{" "}
									{totalDuplicateCount === 1 ? "copy" : "copies"} found
								</span>
								<span className="inline-flex items-center gap-1 text-gray-400 dark:text-gray-500 text-xs font-mono">
									across {totalGroupCount} URL{" "}
									{totalGroupCount === 1 ? "group" : "groups"}
								</span>
							</div>
						)}
					</div>
				...
				<div className="bg-amber-50/50 dark:bg-amber-900/10 border border-amber-200 rounded-2xl p-4 flex gap-3 text-amber-800 dark:text-amber-300">
					...
					<div className="text-xs space-y-1">
						<span className="font-bold block">
							{totalDuplicateCount} redundant{" "}
							{totalDuplicateCount === 1 ? "copy" : "copies"} detected across{" "}
							{totalGroupCount} URL {totalGroupCount === 1 ? "group" : "groups"}
						</span>
						<span>
							Identical URLs stored multiple times bloat your index.{" "}
							<strong>Bulk Auto-Deduplicate keeps the oldest (original)</strong>{" "}
							and removes all newer copies.
						</span>
					</div>
				</div>
			)}
				...
				{duplicateGroups.length === 0 ? (
					<div className="flex flex-col items-center justify-center py-20 bg-gray-50/50 dark:bg-gray-900/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-800">
						...
						<p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
							No duplicates or redunancies detected. Your bookmark collection is
							highly optimized.
						</p>
					</div>
				) : (
					duplicateGroups.map((group) => (
						...
						>
							<div className="bg-gray-50 dark:bg-gray-900 px-5 py-3 border-b border-gray-200 dark:border-gray-800">
								<span className="font-semibold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider block">
									Redundant Path Group
								</span>
								<span className="font-mono text-xs text-blue-600 dark:text-blue-400 break-all mt-1 block font-semibold">
									{group.url}
								</span>
							</div>
							...
							<div className="divide-y divide-gray-100 dark:divide-gray-800">
								{group.items.map((item, idx) => {
									...
									return (
										<div
											key={item.id}
											className="p-5 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-gray-50/40 dark:hover:bg-gray-800/20 transition-all gap-4"
										>
