...
export function OrganizeView() {
  ...
  const [sortField, setSortField] = useState<"date" | "title" | "folder" | "domain">("date");
  ...
  // Pagination Calculations
    ...
    const bulkLock = (lock: boolean) => {
      ...
      batchUpdateBookmarks(updated, lock ? "Bulk Locked bookmarks from AI sort" : "Bulk Unlocked bookmarks for AI sort");
      setSelectedBookmarkIds(new Set());
    };
    ...
    const bulkMoveToFolder = (destFolderId: string | null) => {
      ...
      batchUpdateBookmarks(updated, `Bulk moved bookmarks to folder: ${destFolderId || "Root"}`);
      setSelectedBookmarkIds(new Set());
    ...
    const handleDeleteFolderConfirm = (id: string, name: string) => {
      if (window.confirm(`Are you sure you want to delete folder "${name}"? Subfolders and bookmarks will be moved up.`)) {
        deleteFolder(id);
    ...
    const handleSmartClick = () => {
      ...
      if (targets.length === 0) {
        alert("No bookmarks eligible for Smart Sorting (all bookmarks are already categorized or locked).");
        return;
      }
    ...
    const handleForceClick = () => {
      const targets = bookmarks.filter((b) => includeManual || !b.manuallyAssigned);
      if (targets.length === 0) {
        alert("No bookmarks eligible for Force Sorting (all bookmarks are locked).");
        return;
      }
    ...
    const handleSelectedClick = () => {
      ...
      setReorganizeConfirm({ mode: "selected", targetCount: selectedBookmarkIds.size });
    };

    // Recursive Chrome folder tree renderer
    const renderFolderTreeNode = (node: FolderType, depth = 0) => {
      const hasChildren = subRealFoldersMap.has(node.id) && subRealFoldersMap.get(node.id)!.length > 0;
      ...
      return (
        <div key={node.id} className="select-none">
          <div
            ...
            className={`group flex items-center justify-between py-1.5 px-2 rounded-lg cursor-pointer transition-all ${isSelected
          ...
          >
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              {/* Folder Icon */}
              <Folder size={15} className={isSelected ? "text-blue-500 shrink-0" : "text-gray-400 shrink-0"} />
          ...
          {addingChildToParentId === node.id && (
            <div style={{ paddingLeft: `${(depth + 1) * 14 + 14}px` }} className="py-1 pr-2">
          ...
          {hasChildren && isExpanded && (
            <div className="mt-0.5">
              {subRealFoldersMap.get(node.id)!.map((child) => renderFolderTreeNode(child, depth + 1))}
            </div>
          )}
        </div>
      );
        ...
        <aside className="w-80 flex-shrink-0 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-850 rounded-2xl p-4 flex flex-col h-[calc(100vh-80px)] shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto space-y-1 pr-1 font-sans">
            ...
            <div
              ...
              className={`flex items-center gap-2 py-1.5 px-3 rounded-lg cursor-pointer text-xs transition-colors ${selectedFolderFilter === "all"
                ? "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 font-semibold"
                : "hover:bg-gray-100 dark:hover:bg-gray-850 text-gray-700 dark:text-gray-300"
                }`}
            >
            ...
            <div
              ...
              className={`flex items-center gap-2 py-1.5 px-3 rounded-lg cursor-pointer text-xs transition-colors ${selectedFolderFilter === "uncategorized"
                ? "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 font-semibold"
                : "hover:bg-gray-100 dark:hover:bg-gray-850 text-gray-700 dark:text-gray-300"
                }`}
            >
              <HelpCircle size={14} className="shrink-0 text-gray-400" />
              <span>📂 Uncategorized ({bookmarks.filter(b => b.folderId === null).length})</span>
            </div>
            ...
            {rootRealFolders.length === 0 ? (
              <p className="text-[11px] text-gray-400 italic text-center py-8">No Chrome folders detected.</p>
            ) : (
        ...
        <main className="flex-1 flex flex-col bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-850 rounded-2xl p-5 h-[calc(100vh-80px)] overflow-hidden shadow-sm">
                ...
                <h2 className="text-xl font-bold dark:text-white flex items-center gap-2">
                  ...
                  {selectedFolderFilter !== "all" && (
                    <span className="text-xs font-normal text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded-md">
                      Filter: {selectedFolderFilter === "uncategorized" ? "Uncategorized" : folders.find(f => f.id === selectedFolderFilter)?.name || "Selected Folder"}
                    </span>
                  )}
                </h2>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  Review folder mappings, manually lock entries, and apply AI smart sorting.
                </p>
              </div>
          ...
          {selectedBookmarkIds.size > 0 && (
            <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/40 rounded-xl p-3 shrink-0 my-3 flex items-center justify-between gap-4 flex-wrap text-xs animate-fade-in">
              <span className="font-semibold text-blue-700 dark:text-blue-400">
                Selected {selectedBookmarkIds.size} of {displayedBookmarks.length} items
              </span>
              <div className="flex items-center gap-2 flex-wrap relative">
                ...
                >
                  <Sparkles size={12} className="text-purple-650 dark:text-purple-400" />
                        ...
                        <div className="flex-1 overflow-y-auto space-y-0.5 pr-0.5 scrollbar-thin">
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <Globe size={14} className="text-gray-400 group-hover/item:text-blue-500 shrink-0" />
                              <span className="truncate font-medium">Root / Uncategorized</span>
                            </div>
                          ...
                          {folders
                            .filter(f => f.name.toLowerCase().includes(bulkMoveSearch.toLowerCase()))
                            .map(f => (
                              <button
                                key={f.id}
                                onClick={() => {
                              ...
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <Folder size={14} className="text-gray-400 group-hover/item:text-blue-500 shrink-0" />
              ...
              <thead className="bg-gray-50 dark:bg-gray-850 text-gray-500 uppercase tracking-wider sticky top-0 z-10">
                <tr>
                  <th className="p-3 w-[45px] text-center">
                    ...
                    >
                      {selectedBookmarkIds.size === displayedBookmarks.length && displayedBookmarks.length > 0 ? (
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {isInitialLoading ? (
                  Array.from({ length: 8 }).map((_, idx) => (
                    <tr key={`skeleton-${idx}`} className="animate-pulse border-b border-gray-100 dark:border-gray-800">
                  ...
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-gray-405 italic">
                  ...
                  paginatedBookmarks.map((bm) => {
                    ...
                    return (
                      <tr
                        key={bm.id}
                        className={`hover:bg-gray-50/70 dark:hover:bg-gray-850/40 transition-colors group ${isChecked ? "bg-blue-50/20 dark:bg-blue-900/10" : ""
                          }`}
                      >
                        ...
                        <td className="p-3">
                          <div className="flex flex-col min-w-0 pr-4 whitespace-normal break-words">
                            ...
                            >
                              <span>{bm.title}</span>
                              <ExternalLink size={11} className="opacity-0 group-hover:opacity-100 transition-opacity text-gray-400 shrink-0" />
                            </a>
                        ...
                        <td className="p-3 text-center">
                          <button
                            ...
                            className={`p-1.5 rounded-lg border cursor-pointer inline-flex items-center justify-center transition-all ${bm.manuallyAssigned
                          ...
                          >
                            {bm.manuallyAssigned ? <Lock size={12} /> : <Unlock size={12} />}
                          </button>
                        </td>
                      </tr>
                    );
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between border-t border-gray-100 dark:border-gray-800 pt-4 mt-4 shrink-0 text-xs text-gray-500 dark:text-gray-400 gap-4">
              <div>
                Showing <span className="font-semibold text-gray-700 dark:text-gray-300">{Math.min(totalItems, (currentPage - 1) * itemsPerPage + 1)}</span> to{" "}
                <span className="font-semibold text-gray-700 dark:text-gray-300">{Math.min(totalItems, currentPage * itemsPerPage)}</span> of{" "}
                <span className="font-semibold text-gray-700 dark:text-gray-300">{totalItems}</span> bookmarks
              </div>
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-8 h-8 rounded-lg border flex items-center justify-center font-medium transition-colors cursor-pointer ${currentPage === pageNum
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 max-w-md w-full shadow-2xl space-y-4">
              ...
              <div className="text-sm text-gray-650 dark:text-gray-300 leading-relaxed">
                {reorganizeConfirm.mode === "selected" && (
                  <p>
                    You are about to organize <strong>{reorganizeConfirm.targetCount}</strong> selected bookmarks. This will process only the checked items and place them into folders using AI.
                  </p>
                )}
                {reorganizeConfirm.mode === "force" && (
                  <p>
                    You are about to run a **Force Sorting** on <strong>{reorganizeConfirm.targetCount}</strong> bookmarks. This will analyze and redistribute bookmarks even if they are already in folders. Locked bookmarks will be {includeManual ? "included (reorganized)" : "skipped (safeguarded)"}.
                  </p>
                )}
                {reorganizeConfirm.mode === "smart" && (
                  <p>
                    You are about to run a **Smart Sorting** on <strong>{reorganizeConfirm.targetCount}</strong> bookmarks. This will analyze and organize only bookmarks that are currently uncategorized and unlocked.
                  </p>
                )}
              </div>
              ...
              <div className="bg-blue-50/50 dark:bg-blue-900/10 p-3.5 rounded-xl border border-blue-100 dark:border-blue-900/30 text-xs text-blue-800 dark:text-blue-300">
                Note: This action uses your configured AI endpoint to evaluate bookmark semantic structure and automatically map them to blueprint categories.
              </div>
